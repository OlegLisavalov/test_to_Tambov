...
func (r *newsRepository) UpdateNews(news *models.News) error {
	...
	if len(news.Categories) > 0 {
		...
		_, err := r.db.Exec("DELETE FROM NewsCategories WHERE NewsId = $1", news.Id)
		if err != nil {
		...
		for _, categoryID := range news.Categories {
			_, err := r.db.Exec("INSERT INTO NewsCategories (NewsId, CategoryId) VALUES ($1, $2)", news.Id, categoryID)
			if err != nil {
